...
Given the two samples, we believe that happiness level of one individual would not provide information about the happiness of another individual, making us conclude that these values are independent. 
...
```
Assuming that the samples are splitted based on the GDP per capita mean value of 9.584, we found that both samples sizes exceed 30, so the Central Limit Theorem approximates a normal distribution.

For this reason, we conclude that there are no major deviations from normality, considering the sample size.
...
```
The senator age is calculated, using the current year 2022 minus the birthyear, as a discrete value from 0 to 100. There is no continuous relationship between two age interval (ie. 89, 90 are two distinct age). For that reason, the assumption that the age is on a ordinal scale is met.
...
By viewing the format of the wine dataset, the columns heart and liver are denoted as heart disease deaths per 100,000 and liver disease deaths per 100,000, respectively. Since the two columns both measure death per 100,000 individuals, it is measuring a continous average number of death and is measuring from the same scale. Therefore the assumption that both samples are metric is met.


2) IID data.
  - In particular, each pair $(X_i,Y_i)$ is drawn from the same distribution, independently of all other pairs.

While it might be true that the death by heart disease provide no information toward the death by liver disease. We must consider that when a sample pair of (death by heart disease, death by liver disease) is drawn for a country, that country could potentially provide provide some information to it neighboring country. Perhaps several countries are in a highly polluted area. This established a dependency and violate the IID assumption. Therefore, IID does not met here 
...
sum(!is.na(wine$diff))
hist(wine$heart-wine$liver, main='Differences between Heart/Liver Death', breaks=5)
```
Here the number of observations for the differences between death by heart disease and death by liver disease is calculated to be n=21. This put it under n>30 boundary for the Central Limit Theorem. Additionally, when plotted in a histogram with number of bins=5, we see that the distribution does not really show a normal distribution. For those reason, the assumption that the distribution of the difference $X - Y$ is symmetric around some mean $\mu$ is not met here.
...
To assess the sample as being a metric scale, we need to see whether or not the data are presented with a degree of difference between categories. From the description of the feeling thermometer, a rating from 0 to 100, a rating between 50 degrees and 100 degrees mean that you feel favorable and warm toward the group and a rating between 0 degrees and 50 degrees means that you don't feel favorable toward the group. Theoretically, if we look at the interval [49,51] we can see that any values between 50 to 51 degrees fall into the feeling favorable group while any values between 49 to 50 fall into the feeling unfavorable group. Using that logic, we can see that there is a degree of difference between categories and therefore the assumption of metric scale is met.
...
```
The description of the GSS questions suggested that the feeling thermoneter is a rating from 0 to 100. The rating is obtain by asking the participant to give a rating to name. Keeping this in mind when viewing the summary of the two samples in this dataset, prottemp and cathtemp, we see that they both has mininum value of 0 and maxium value of 100. Additioanlly, the inner quartile range, computed by taking the difference between the 3rd Quartile and the 1st Quartile, is equal to 35 for both samples. Assessing from this angle, each of the two sample pair (X_i, Y_i) is drawn from the same distribution ranging from a scale of 0 to 100. Therefore they are identically distributed. In term of independent, because the sample pair (X_i, Y_i), or an individual raiting on a protestant group and catholic group, provides no information on another user rating for the two groups. It is safe to say that each sample pair is independent. For these reason, we believe that the assumption of IID data is met.
...
```{r}
GSS_religion$diff <- GSS_religion$prottemp-GSS_religion$cathtemp
sum(!is.na(GSS_religion$diff))
```
```{r}
hist(GSS_religion$diff, main='Differences between Protemp and Cathtemp', breaks=25)
```


To assess whether or not the difference between the two measurements has no major deviation from normality, we first need to compute the differences between the two samples and count the number of non-nan values in the resulted differences column. Here we see that the sample size is n=802, which exceed a lower bound of n=30. For that reason the Central Limit Theorem approximates a normal distribution. Viewing the histogram with number of bins(breaks) = 25, calculated by the taking the square root of the sample size, we see that the sample is relatively distributed with center at approximately 0. For these reasons, the assumption of no deviation from normality is met.
